...
public class KillHandler
{
    ...
    public void onLivingDeath(LivingDeathEvent event)
    {
        ...
        if(debugMode)
        {
            ...
            GunMod.LOGGER.info("[CGM-DEBUG]   victim      : {} ({})", victim.getName().getString(), victim.getClass().getName());
            ...
            if(killer != null)
            {
                GunMod.LOGGER.info("[CGM-DEBUG]   killer      : {} ({})", killer.getName().getString(), killer.getClass().getName());
                GunMod.LOGGER.info("[CGM-DEBUG]   killer.isPlayer: {}", killer instanceof PlayerEntity);
            }
            else
            {
        ...
        if(!(victim instanceof PlayerEntity) || !(killer instanceof PlayerEntity))
            return;
        ...
        String msg = String.format("[CGM] \"%s\" killed \"%s\" at \"%d, %d, %d\" at \"%s\"",
        ...
        StringTextComponent component = new StringTextComponent(TextFormatting.GOLD + msg);
    ...
    private void triggerKillEffect(ServerPlayerEntity killer, Entity victim)
    {
        ...
        boolean hasEffect = KillEffectListener.aceEffectPlayers.contains(uuid)
        ...
        // Kill confirmation sound — audible to everyone within ~20 blocks
        world.playSound(null, pos, ModSounds.KILL_EFFECT_KILL.get(), SoundCategory.PLAYERS, 1.25F, 1.0F);
