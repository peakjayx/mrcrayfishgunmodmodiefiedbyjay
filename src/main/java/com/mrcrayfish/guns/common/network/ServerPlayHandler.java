...
public class ServerPlayHandler
{
    ...
    public static void handleFireMode(MessageFireMode message, ServerPlayerEntity player)
    {
        ItemStack held = player.getMainHandItem();
        if(held.getItem() instanceof SwitchGunItem)
        {
            int mode = net.minecraft.util.math.MathHelper.clamp(message.getMode(), 0, 2);
            SwitchGunItem.setFireMode(held, mode);
        }
    }

    public static void handleToggleHelm(
        com.mrcrayfish.guns.network.message.MessageToggleHelm msg,
        ServerPlayerEntity player)
    {
        ItemStack helm = player.getItemBySlot(net.minecraft.inventory.EquipmentSlotType.HEAD);
        if(!(helm.getItem() instanceof com.mrcrayfish.guns.api.IHelm)) return;
        com.mrcrayfish.guns.api.IHelm helmItem = (com.mrcrayfish.guns.api.IHelm) helm.getItem();

        net.minecraft.nbt.CompoundNBT tag = helm.getOrCreateTag();

        if(msg.getType() == com.mrcrayfish.guns.network.message.MessageToggleHelm.NV)
        {
            boolean nv = !helmItem.isNvActive(helm);
            helmItem.setNvActive(helm, nv);
            if(nv)
            {
                player.addEffect(new net.minecraft.potion.EffectInstance(
                    net.minecraft.potion.Effects.NIGHT_VISION, 999999, 0, false, false));
            }
            else
            {
                player.removeEffect(net.minecraft.potion.Effects.NIGHT_VISION);
            }
        }
        else
        {
            helmItem.setVisierDown(helm, !helmItem.isVisierDown(helm));
        }
    ...
    public static void handleToggleLight(
        com.mrcrayfish.guns.network.message.MessageToggleLight msg,
        ServerPlayerEntity player)
    {
        ItemStack held = player.getMainHandItem();
        ...
        net.minecraft.item.Item flashlight = com.mrcrayfish.guns.init.ModItems.FLASHLIGHT.get();
        net.minecraft.item.Item redDot     = com.mrcrayfish.guns.init.ModItems.RED_DOT.get();

        if (msg.getType() == com.mrcrayfish.guns.network.message.MessageToggleLight.FLASHLIGHT)
        {
            net.minecraft.item.ItemStack l = Gun.getAttachment(com.mrcrayfish.guns.item.attachment.IAttachment.Type.SIDE_LEFT,    held);
            net.minecraft.item.ItemStack r = Gun.getAttachment(com.mrcrayfish.guns.item.attachment.IAttachment.Type.SIDE_RIGHT,   held);
            net.minecraft.item.ItemStack u = Gun.getAttachment(com.mrcrayfish.guns.item.attachment.IAttachment.Type.UNDER_BARREL, held);
            hasAttachment = (!l.isEmpty() && l.getItem() == flashlight)
                         || (!r.isEmpty() && r.getItem() == flashlight)
                         || (!u.isEmpty() && u.getItem() == flashlight);
        }
        else
        {
            net.minecraft.item.ItemStack l = Gun.getAttachment(com.mrcrayfish.guns.item.attachment.IAttachment.Type.SIDE_LEFT,    held);
            net.minecraft.item.ItemStack r = Gun.getAttachment(com.mrcrayfish.guns.item.attachment.IAttachment.Type.SIDE_RIGHT,   held);
            net.minecraft.item.ItemStack u = Gun.getAttachment(com.mrcrayfish.guns.item.attachment.IAttachment.Type.UNDER_BARREL, held);
            hasAttachment = (!l.isEmpty() && l.getItem() == redDot)
                         || (!r.isEmpty() && r.getItem() == redDot)
                         || (!u.isEmpty() && u.getItem() == redDot);
        }
        if (!hasAttachment) return;

        String nbtKey = msg.getType() == com.mrcrayfish.guns.network.message.MessageToggleLight.FLASHLIGHT
            ? "FlashlightOn" : "LaserOn";
        net.minecraft.nbt.CompoundNBT tag = held.getOrCreateTag();
        tag.putBoolean(nbtKey, !tag.getBoolean(nbtKey));
        player.inventoryMenu.broadcastChanges();
    }
}
