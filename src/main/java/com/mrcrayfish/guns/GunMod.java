...
import com.mrcrayfish.guns.client.FlashlightLightIntegration;
...
public class GunMod
{
    ...
    {
        GunDebugCommand.register(event.getDispatcher());
        KillEffectCommand.register(event.getDispatcher());
    }
    ...
    private void onClientSetup(FMLClientSetupEvent event)
    {
        ...
        event.enqueueWork(() ->
        {
            ...
            net.minecraft.item.ItemModelsProperties.register(
                ModItems.RED_DOT.get(),
                new ResourceLocation(Reference.MOD_ID, "laser_on"),
                (stack, world, entity) -> {
                    if (entity == null) return 0.0F;
                    net.minecraft.item.ItemStack gun = entity.getMainHandItem();
                    if (!(gun.getItem() instanceof com.mrcrayfish.guns.item.GunItem)) return 0.0F;
                    return gun.getOrCreateTag().getBoolean("LaserOn") ? 1.0F : 0.0F;
                }
            );
            if (ModList.get().isLoaded("ryoamiclights"))
            {
                FlashlightLightIntegration.register();
            }
        });
    }
