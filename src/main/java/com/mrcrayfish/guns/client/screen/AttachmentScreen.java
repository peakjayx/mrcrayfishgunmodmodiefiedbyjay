...
public class AttachmentScreen extends ContainerScreen<AttachmentContainer>
{
    ...
    private static final ResourceLocation SIDE_LEFT_ICON  = new ResourceLocation("cgm:textures/gui/laser.png");
    private static final ResourceLocation SIDE_RIGHT_ICON = new ResourceLocation("cgm:textures/gui/flashlight_right.png");

    // Anzahl der Haupt-Slots (Scope/Barrel/Stock/UnderBarrel)
    ...
    public AttachmentScreen(AttachmentContainer screenContainer, PlayerInventory playerInventory, ITextComponent titleIn)
    {
        ...
        this.imageHeight = 184; // Originalwert — nicht dynamisch
    }
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        // Tooltips für die 4 Haupt-Slots
            ...
            if(RenderUtil.isMouseWithin(mouseX, mouseY, startX + 7, startY + 16 + i * 18, 18, 18))
            {
                ...
                if(!this.menu.getSlot(i).isActive())
                {
                    this.renderComponentTooltip(matrixStack, Arrays.asList(
                        new TranslationTextComponent("slot.cgm.attachment." + type.getTranslationKey()),
                        new TranslationTextComponent("slot.cgm.attachment.not_applicable")), mouseX, mouseY);
                }
                else if(this.weaponInventory.getItem(i).isEmpty())
                {
                    this.renderComponentTooltip(matrixStack,
                        Collections.singletonList(new TranslationTextComponent("slot.cgm.attachment." + type.getTranslationKey())),
                        mouseX, mouseY);
                }
            }
        }

        // Tooltips für Side-Slots (links neben dem Panel)
        int sideX = startX + AttachmentContainer.SIDE_X;
        for(int i = 0; i < 2; i++)
        {
            int typeIdx = MAIN_SLOT_COUNT + i;
            if(RenderUtil.isMouseWithin(mouseX, mouseY, sideX - 1, startY + 16 + i * 18, 18, 18))
            {
                IAttachment.Type type = IAttachment.Type.values()[typeIdx];
                if(!this.menu.getSlot(typeIdx).isActive())
                {
                    this.renderComponentTooltip(matrixStack, Arrays.asList(
                        new TranslationTextComponent("slot.cgm.attachment." + type.getTranslationKey()),
                        new TranslationTextComponent("slot.cgm.attachment.not_applicable")), mouseX, mouseY);
                }
                else if(this.weaponInventory.getItem(typeIdx).isEmpty())
                {
                    this.renderComponentTooltip(matrixStack,
                        Collections.singletonList(new TranslationTextComponent("slot.cgm.attachment." + type.getTranslationKey())),
                        mouseX, mouseY);
    ...
    protected void renderBg(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY)
    {
        ...
        // Side-Slots — eigene Hintergrundboxen links neben dem Panel zeichnen
        this.renderSideSlots(matrixStack, left, top);
    }

    private void renderSideSlots(MatrixStack matrixStack, int left, int top)
    {
        int sideX = left + AttachmentContainer.SIDE_X;
        ResourceLocation[] icons = {SIDE_LEFT_ICON, SIDE_RIGHT_ICON};

        for(int i = 0; i < 2; i++)
        {
            int slotY = top + 17 + i * 18;
            int typeIdx = MAIN_SLOT_COUNT + i;

            // Slot-Hintergrund (Vanilla-Style)
            fill(matrixStack, sideX - 1, slotY - 1, sideX + 17, slotY + 17, 0xFF6B6B6B);
            fill(matrixStack, sideX,     slotY,     sideX + 16, slotY + 16, 0xFF373737);

            // Icon aus separater PNG — immer anzeigen wenn Slot leer oder inaktiv
            if(this.weaponInventory.getItem(typeIdx).isEmpty())
            {
                RenderSystem.color4f(1.0F, 1.0F, 1.0F, this.menu.getSlot(typeIdx).isActive() ? 1.0F : 0.4F);
                Minecraft.getInstance().getTextureManager().bind(icons[i]);
                // blit mit expliziter Textur-Größe (16x16 PNG)
                blit(matrixStack, sideX, slotY, 0, 0, 16, 16, 16, 16);
                RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
            }
        }
    }
