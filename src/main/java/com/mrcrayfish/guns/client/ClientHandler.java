...
import com.mrcrayfish.guns.client.handler.FireModeHandler;
import com.mrcrayfish.guns.client.handler.FlashlightHandler;
import com.mrcrayfish.guns.client.handler.LaserRenderer;
import com.mrcrayfish.guns.client.renderer.layer.VestLayer;
...
public class ClientHandler
{
    ...
    public static void registerVestLayer()
    {
        try
        {
            Object erd = Minecraft.getInstance().getEntityRenderDispatcher();
            //noinspection rawtypes
            Field field = ObfuscationReflectionHelper.findField((Class) erd.getClass(), "field_178636_l");
            java.util.Map<String, PlayerRenderer> skinMap =
                (java.util.Map<String, PlayerRenderer>) field.get(erd);
            skinMap.values().forEach(r -> r.addLayer(new VestLayer(r)));
        }
        catch(Exception e)
        {
            GunMod.LOGGER.warn("VestLayer registration failed: " + e.getMessage());
        }
    }
